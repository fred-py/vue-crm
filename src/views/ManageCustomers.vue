...
const selectedCustomers = ref([]);  // Initialise an empty array  Refer to v-model under DataTable - this selects customers on teh datarable
const selectedCusField = ref('');
//const customers = ref([]);
...
//watch(customer, (newName, oldName) => {
//    if (newName.name !== oldName.name) {
//        selectedCustomers.value = newName.name; // Update selectedCustomers with new name
//    }
//});

//const canSave = computed(() => {
//  return customer.name.value; // Adjust conditions as needed
//});
...
const editCustomer = (editCustomer) => {
    customer.value = editCustomer; // Update customer object
...
const saveCustomer = (customerData) => {
    // This will trigger the axios PUT request to update database
          ...
          <div class="card">
              ...
              <Dialog v-model:visible="customerDialog" :style="{ width: '450px' }" header="Customer Details" :modal="true" class="p-fluid">
                  <div class="field">
                      ...
                      <InputText id="name" v-model="customer.name" required="true" autofocus :invalid="submitted && !customer.name" />
                      <small class="p-invalid" v-if="submitted && !customer.name">Name is required.</small>
                  </div>
                
                  <div class="field">
                      <label for="street">Street</label>
                      <InputText id="street" v-model="customer.street" required="true" rows="3" cols="20" />
                  </div>
                  <div class="formgrid grid">
                      <div class="field col">
                          <label for="city">City</label>
                          <InputText id="city" v-model="customer.city" :invalid="submitted && !customer.city" :required="true" />
                          <small class="p-invalid" v-if="submitted && !customer.phone">Phone is required.</small>
                      </div>
                      <div class="field col">
                          <label for="postcode">Postcode</label>
                          <InputText id="postcode" v-model="customer.postcode" integeronly />
                      </div>
                  </div> 
              ...
              <Dialog v-model:visible="deleteCustomerDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
                  ...
                  <template #footer>
                      ...
                      <Button label="Cancel" icon="pi pi-times" text="" @click="hideDialog" />
                      <Button label="Save" icon="pi pi-check" text="" @click="saveCustomer" :disabled="!canSave" />
                  </template>
              </Dialog>
